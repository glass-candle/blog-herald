RSpec.describe Presentation::Bot::Actions::CallbackQuery::Subscribe, type: :unit_test do
  subject do
    described_class.new(subscribe: subscribe)
  end

  include Dry::Effects::Handler::Reader(:action_processor)

  let(:subscribe) { instance_double('Application::Operations::Chat::Subscribe') }

  let(:action_processor) { instance_double('Presentation::Bot::ActionHandler::CallbackQueryProcessor') }

  let(:chat_id) { '123456' }
  let(:message_id) { '123' }
  let(:path) { "subscribe:#{codename}|#{callback_path}" }
  ...
  describe 'with all operations successful' do
    before do
      ...
      allow(subscribe).to receive(:call).with(chat_id, codename).and_return(Dry::Monads::Result::Success.new(double))
    end
  ...
  describe 'with a failed operation' do
    before do
      allow(subscribe).to receive(:call).with(chat_id, codename).and_return(Dry::Monads::Result::Failure.new(:not_ok))
    end
  ...
  describe 'with an invalid path' do
    let(:path) { "subscribe:#{codename}|gibberish" }
