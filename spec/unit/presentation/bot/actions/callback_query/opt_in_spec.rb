RSpec.describe Presentation::Bot::Actions::CallbackQuery::OptIn, type: :unit_test do
  subject do
    described_class.new(
      opt_in: opt_in,
  ...
  let(:opt_in) { instance_double('Application::Operations::Chat::OptIn') }
  ...
  describe 'with all operations successful' do
    ...
    before do
      ...
      allow(opt_in).to receive(:call).with(chat_id).and_return(Dry::Monads::Result::Success.new({}))
  ...
  describe 'with a failed operation' do
    before do
      allow(opt_in).to receive(:call).with(chat_id).and_return(Dry::Monads::Result::Failure.new(:not_ok))
    end
